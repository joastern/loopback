...
module.exports = function(registry) {
  ...
  PersistedModel.setupRemoting = function() {
    ...
    // This is the defualt verb used for 3.X,
    // whereas the default values are the other way for 2.X
    var configurableVerb = {
      updateAttributes: 'patch',
      updateOrCreate: 'patch',
      replaceOrCreate: 'put',
      replaceById: 'put',
    };
    // Just for passing CI for now
    options.oldMapping = true;
    // we check options.newMapping when backporting to `2.x`. Thought?
    if (options.oldMapping) {
      configurableVerb.replaceById = 'patch';
      configurableVerb.replaceOrCreate = 'patch';
      configurableVerb.updateAttributes = 'put';
      configurableVerb.updateOrCreate = 'put';
    }
